...
# Install Node.js
echo "Installing Node.js as plex-iptv-proxy user"
su -s /bin/bash -c '
if command -v curl &> /dev/null; then
    ...
    echo "Error: Neither curl nor wget is installed, cannot install nvm"
    exit 1
fi
...
# Modify the unit files to use the INSTALL_DIR path
